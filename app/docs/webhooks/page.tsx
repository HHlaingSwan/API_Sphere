...
const events = [
  { event: "payment.succeeded", category: "Payment", description: "A payment was successfully processed." },
  { event: "payment.failed", category: "Payment", description: "A payment attempt failed." },
  { event: "customer.created", category: "Customer", description: "A new customer account was created." },
  { event: "customer.updated", category: "Customer", description: "Customer profile information changed." },
  { event: "invoice.paid", category: "Invoice", description: "An invoice was paid successfully." },
  { event: "invoice.payment_failed", category: "Invoice", description: "An invoice payment attempt failed." },
];
...
export default function WebhooksDocsPage() {
  return (
    <DocPageLayout protocolKey="webhooks">
      <div>
        <span className="text-xs font-semibold text-orange-400 uppercase tracking-wider">{doc.category}</span>
      ...
      <div className="rounded-2xl border border-border bg-muted/15 p-4 sm:p-6">
        <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">Live Event Feed</h3>
          ...
          {events.map((ev, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <code className="text-xs font-mono text-foreground font-semibold">{ev.event}</code>
              </div>
              <div className="flex items-center gap-3">
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${categoryColors[ev.category] || "bg-muted text-muted-foreground border-border"}`}>
                  {ev.category}
                </span>
      ...
      <div className="rounded-2xl border border-border bg-muted/15 p-4 sm:p-6">
        <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">Webhook Flow</h3>
        <div className="flex items-center justify-between px-4 py-6">
          ...
          <div className="flex-1 mx-4 flex flex-col items-center gap-1">
            <span className="text-[10px] font-mono text-orange-400">POST /webhooks/stripe →</span>
            <div className="w-full h-[2px] bg-gradient-to-r from-orange-500/50 to-emerald-500/50 rounded-full" />
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <span className="text-xs font-bold text-emerald-400">Your Server</span>
            <span className="text-[10px] text-muted-foreground">/webhooks/stripe</span>
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border bg-background/40 p-5">
        ...
        <div className="flex flex-col gap-2">
          {events.map((ev, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-2 rounded-lg bg-muted/30 border border-border/40">
              <code className="text-xs font-mono text-foreground font-semibold shrink-0">{ev.event}</code>
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border shrink-0 ${categoryColors[ev.category] || "bg-muted text-muted-foreground border-border"}`}>
                {ev.category}
              </span>
