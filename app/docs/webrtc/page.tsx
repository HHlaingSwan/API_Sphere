...
const signalingMessages = [
  {
    type: "offer",
    description: "The caller sends an SDP offer to initiate a session.",
    direction: "Client → Server",
  },
  {
    type: "answer",
    description:
      "The callee responds with an SDP answer accepting the session.",
    direction: "Server → Client",
  },
  {
    type: "candidate",
    description: "ICE candidates exchanged for NAT traversal.",
    direction: "Bidirectional",
  },
  {
    type: "bye",
    description: "A peer signals it is leaving the session.",
    direction: "Client → Server",
  },
];
      ...
      <div className="rounded-2xl border border-border bg-muted/15 p-4 sm:p-6">
        ...
        <div className="flex items-center justify-between px-4 py-6">
          ...
          <div className="flex-1 mx-4 flex flex-col items-center gap-2">
            ...
            <div className="w-full h-0.5 bg-linear-to-r from-rose-500/50 via-violet-500/50 to-blue-500/50 rounded-full" />
            <span className="text-[10px] font-mono text-blue-400">
              ← SDP Answer
            </span>
            <div className="w-full h-0.5 bg-linear-to-r from-blue-500/50 via-violet-500/50 to-rose-500/50 rounded-full" />
        ...
        <div className="flex flex-col gap-2">
          {signalingMessages.map((m, i) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${directionColors[m.direction]}`}
                >
                  {m.direction}
                </span>
                <span className="text-xs text-muted-foreground max-w-55 text-right hidden sm:block">
                  {m.description}
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="rounded-lg border border-rose-500/20 bg-rose-500/5 p-3">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Interactive Connectivity Establishment — finds the best path
              between peers.
            </p>
          </div>
          <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 p-3">
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Session Traversal Utilities for NAT — discovers public IP/port.
            </p>
          </div>
          <div className="rounded-lg border border-cyan-500/20 bg-cyan-500/5 p-3">
            <span className="text-xs font-bold text-cyan-400">TURN</span>
            <p className="text-[11px] text-muted-foreground mt-1">
              Relay server fallback when direct P2P fails (symmetric NAT).
            </p>
          </div>
        </div>
      </div>
