...
const channels = [
  ...
  { channel: "cursor-move", description: "Collaborative cursor position sharing.", direction: "Client → Server" },
  { channel: "chat", description: "Live chat messages between users.", direction: "Bidirectional" },
  { channel: "typing", description: "Typing indicator for chat applications.", direction: "Bidirectional" },
  { channel: "notifications", description: "Push notifications for alerts and mentions.", direction: "Server → Client" },
];
...
export default function WebsocketsDocsPage() {
  return (
    <DocPageLayout protocolKey="websockets">
      <div>
        <span className="text-xs font-semibold text-cyan-400 uppercase tracking-wider">{doc.category}</span>
      ...
      <div className="rounded-2xl border border-border bg-muted/15 p-4 sm:p-6">
        <h3 className="text-xs font-bold uppercase tracking-wider text-muted-foreground mb-4">Persistent Connection Tunnel</h3>
        <div className="flex items-center justify-between px-4 py-6">
          ...
          <div className="flex-1 mx-4 flex flex-col items-center gap-2">
            <div className="w-full h-3 rounded-full bg-cyan-500/20 border border-cyan-500/30 overflow-hidden relative">
              <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/40 via-transparent to-cyan-500/40 animate-pulse" />
            </div>
            <div className="flex items-center gap-2 text-[10px] font-mono">
              <span className="text-cyan-400">◄═══════════════════════════►</span>
            </div>
            <span className="text-[10px] text-muted-foreground">ws:// → Upgrade → 101 → Open</span>
          </div>
      ...
      <div className="rounded-xl border border-border bg-background/40 p-5">
        <h3 className="text-sm font-bold text-foreground mb-3">Channels & Direction</h3>
          ...
          {channels.map((ch, i) => (
            <div key={i} className="flex items-center justify-between px-3 py-2.5 rounded-lg bg-muted/30 border border-border/40">
              <div className="flex items-center gap-3">
                ...
                <code className="text-xs font-mono text-foreground font-semibold">{ch.channel}</code>
              </div>
              <div className="flex items-center gap-3">
                <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${directionColors[ch.direction]}`}>
                  {ch.direction}
                </span>
                <span className="text-xs text-muted-foreground max-w-[200px] text-right hidden sm:block">{ch.description}</span>
              </div>
            </div>
          ))}
        </div>
